...
export async function GET(request:Request){
  ...
  try{
    ...
    const pdf=createMaintenancePdf({title:base.kind==="IMPORTED"?"Histórico importado":"Comprovante de manutenção",equipment:String(equipment?.prefix??base.prefix),category:base.category,brand:String(equipment?.brand??""),model:String(equipment?.model??""),date:`${formatPdfDate(base.date)}${base.isGenericDate?" (Data genérica — histórico importado sem data original)":""}`,reading,services:services.map((item)=>item.service),notes:base.notes??"Sem observações.",responsible:base.responsible,workOrder:base.workOrder,generatedAt:formatPdfDate(new Date().toISOString())});
