...
export default function Home() {
    ...
    const contentProps = { data, authUser, refresh, flash, taskRoles, loadTaskRoles };
...
function UsersView({ users, authUser, loadUsers, flash, openUserModal, taskRoles, loadTaskRoles }: {
    users: UserRecord[];
    authUser: AuthUser;
    loadUsers: () => Promise<void>;
    flash: (message: string) => void;
...
function UserModal({ item, actor, fronts, taskRoles, close, saved }: {
    ...
    close: () => void;
    saved: (message: string) => void;
}) { const baseProfile = (item?.profile === "ALMOXARIFADO" ? "OPERADOR" : item?.profile ?? "OPERADOR") as "ADMIN" | "GESTOR" | "OFICINA" | "OPERADOR"; const [profile, setProfile] = useState(baseProfile); const [permissions, setPermissions] = useState<Permission[]>(item?.permissions ?? profileDefaults[baseProfile]); const [taskRoleId, setTaskRoleId] = useState(item?.taskRoleId ? String(item.taskRoleId) : ""); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const canEditTaskRole = actor.profile === "ADMIN" && item?.id !== actor.id; const selectedTaskRole = taskRoles.find((role) => role.id === Number(taskRoleId)); async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(""); const form = new FormData(event.currentTarget); const payload = { id: item?.id, action: "UPDATE", name: form.get("name"), username: form.get("username"), email: form.get("email"), password: item ? undefined : form.get("password"), profile, taskRoleId: taskRoleId ? Number(taskRoleId) : null, status: item?.status ?? "ACTIVE", serviceFrontId: form.get("serviceFrontId") || null, permissions }; try {
