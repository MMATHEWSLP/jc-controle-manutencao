...
type HierarchyLevel = "ADMIN" | "GESTOR" | "SUB1" | "SUB2" | "SUB3" | "USUARIO";
const HIERARCHY_LEVELS: HierarchyLevel[] = ["ADMIN", "GESTOR", "SUB1", "SUB2", "SUB3", "USUARIO"];
const hierarchyLabels: Record<HierarchyLevel, string> = { ADMIN: "Admin", GESTOR: "Gestor", SUB1: "Sub 1", SUB2: "Sub 2", SUB3: "Sub 3", USUARIO: "Usuário" };
type AuthUser = {
    id: number;
    name: string;
    username: string;
...
} } const [subView, setSubView] = useState<"list" | "hierarchy">("list"); return <><ModuleHeader eyebrow="ACESSO E SEGURANÇA" title="Usuários e permissões" subtitle="Este módulo permanece separado de Troca de Óleo." action={can(authUser, "users.create") ? "＋ Novo usuário" : undefined} onAction={() => openUserModal("new")}/><div className="user-summary"><article><strong>{users.length}</strong><span>Cadastrados</span></article><article><strong>{users.filter((user) => user.status === "ACTIVE").length}</strong><span>Ativos</span></article><article><strong>{users.filter((user) => user.profile === "ADMIN").length}</strong><span>Administradores</span></article><article><strong>{users.filter((user) => user.status === "INACTIVE").length}</strong><span>Inativos</span></article></div><div className="main-tabs secondary-module-nav" aria-label="Sub-navegação de Usuários"><button className={subView === "list" ? "active" : ""} onClick={() => setSubView("list")}>Lista</button><button className={subView === "hierarchy" ? "active" : ""} onClick={() => setSubView("hierarchy")}>Hierarquia</button></div>{subView === "list" ? <article className="panel module-panel"><div className="table-scroll"><table className="users-table"><thead><tr><th>Nome</th><th>Usuário</th><th>E-mail</th><th>Frente</th><th>Perfil</th><th>Nível hierárquico</th><th>Status</th><th>Último acesso</th><th>Ações</th></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td><div className="user-name-cell"><span className="avatar light">{user.name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</span><div><strong>{user.name}</strong>{user.isPrimaryAdmin && <small>Administrador principal</small>}</div></div></td><td>@{user.username}</td><td>{user.email}</td><td>{user.serviceFrontName ?? "Todas as frentes"}</td><td><span className="role-pill">{user.profileLabel}</span></td><td><span className="role-pill">{user.hierarchyLevelLabel}</span></td><td><span className={`status-pill ${user.status === "ACTIVE" ? "green" : "gray"}`}>{user.status === "ACTIVE" ? "Ativo" : "Inativo"}</span></td><td>{user.lastAccessAt ? formatDate(user.lastAccessAt) : "Nunca"}</td><td><div className="user-actions">{can(authUser, "users.edit") && (!user.isPrimaryAdmin || user.id === authUser.id) && <button onClick={() => openUserModal(user)}>Editar</button>}{can(authUser, "users.status") && !user.isPrimaryAdmin && user.id !== authUser.id && <button className={user.status === "ACTIVE" ? "danger-action" : "activate-action"} onClick={() => toggle(user)}>{user.status === "ACTIVE" ? "Desativar" : "Ativar"}</button>}</div></td></tr>)}</tbody></table></div></article> : <HierarchyView users={users} authUser={authUser} openUserModal={openUserModal}/>}</>; }

function HierarchyView({ users, authUser, openUserModal }: {
    users: UserRecord[];
    authUser: AuthUser;
    openUserModal: (item: UserRecord) => void;
}) { const active = users.filter((user) => user.status === "ACTIVE"); return <article className="panel module-panel hierarchy-panel">{HIERARCHY_LEVELS.map((level) => { const group = active.filter((user) => user.hierarchyLevel === level); return <section className="hierarchy-group" key={level}><header><h3>{hierarchyLabels[level]}</h3><span>{group.length} usuário(s)</span></header><div className="hierarchy-group-users">{group.map((user) => <div className="hierarchy-user-card" key={user.id}><span className="avatar light">{user.name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</span><div><strong>{user.name}</strong><small>{user.profileLabel}{user.serviceFrontName ? ` · ${user.serviceFrontName}` : ""}</small></div>{can(authUser, "users.edit") && <button onClick={() => openUserModal(user)}>Editar</button>}</div>)}{group.length === 0 && <div className="empty-state">Nenhum usuário neste nível.</div>}</div></section>; })}</article>; }
...
function UserModal({ item, actor, fronts, close, saved }: {
    ...
    close: () => void;
    saved: (message: string) => void;
}) { const baseProfile = (item?.profile === "ALMOXARIFADO" ? "OPERADOR" : item?.profile ?? "OPERADOR") as "ADMIN" | "GESTOR" | "OFICINA" | "OPERADOR"; const [profile, setProfile] = useState(baseProfile); const [permissions, setPermissions] = useState<Permission[]>(item?.permissions ?? profileDefaults[baseProfile]); const [hierarchyLevel, setHierarchyLevel] = useState<HierarchyLevel>(item?.hierarchyLevel ?? "USUARIO"); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const canEditHierarchy = actor.profile === "ADMIN" && item?.id !== actor.id; async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(""); const form = new FormData(event.currentTarget); const payload = { id: item?.id, action: "UPDATE", name: form.get("name"), username: form.get("username"), email: form.get("email"), password: item ? undefined : form.get("password"), profile, hierarchyLevel, status: item?.status ?? "ACTIVE", serviceFrontId: form.get("serviceFrontId") || null, permissions }; try {
