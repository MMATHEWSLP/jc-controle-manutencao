...
type Section = "Dashboard" | "Equipamentos da troca" | "Equipamentos" | "QR Codes" | "Horímetros / KM" | "Registrar troca de óleo" | "Central de alertas" | "WhatsApp" | "Histórico" | "Status da Frota" | "Solicitação de Materiais" | "Produtos" | "Tarefas" | "Usuários";
...
const permissionGroups: Array<{
    ...
    { label: "Frentes de Serviço", items: [["service_fronts.manage", "Cadastrar, renomear e ativar/desativar frentes"]] },
];
const allPermissions = permissionGroups.flatMap((group) => group.items.map(([key]) => key));
const profileDefaults: Record<"ADMIN" | "GESTOR" | "OFICINA" | "OPERADOR", Permission[]> = { ADMIN: allPermissions, GESTOR: ["dashboard.view", "equipment.view", "meter.view", "maintenance.view", "maintenance.history", "alerts.view", "alerts.share", "whatsapp.view", "whatsapp.send", "fleet.view", "fleet.update", "fleet.report", "materials.view", "materials.manage", "tasks.view", "tasks.create", "tasks.edit", "products.view", "products.create", "products.edit", "suppliers.view", "suppliers.create", "suppliers.edit"], OFICINA: ["equipment.view", "equipment.edit_plan", "meter.view", "meter.create", "maintenance.view", "maintenance.create", "maintenance.edit", "maintenance.history", "alerts.view", "fleet.view", "fleet.update", "fleet.report", "materials.view", "materials.request", "tasks.view", "tasks.create", "tasks.edit"], OPERADOR: [] };
...
export default function Home() {
    ...
    const [activeFrontId, setActiveFrontId] = useState<number | "ALL">("ALL");
    const [taskRoles, setTaskRoles] = useState<TaskRole[]>([]);
    const loadTaskRoles = useCallback(async () => { try { const result = await fetchJson<{ roles: TaskRole[] }>("/api/task-roles"); setTaskRoles(result.roles); } catch { setTaskRoles([]); } }, []);
    ...
    useEffect(() => { if (authUser)
        loadTaskRoles(); }, [authUser, loadTaskRoles]);
    useEffect(() => { if (!authUser)
        return; const saved = window.localStorage.getItem(`maintenance-active-front-${authUser.id}`); if (saved)
        setActiveFrontId(saved === "ALL" ? "ALL" : Number(saved)); }, [authUser]);
    useEffect(() => { if (authUser)
        window.localStorage.setItem(`maintenance-active-front-${authUser.id}`, String(activeFrontId)); }, [authUser, activeFrontId]);
    const visibleFronts = useMemo(() => { if (!authUser)
        return []; if (authUser.profile === "ADMIN" || authUser.allServiceFronts)
        return serviceFronts; return serviceFronts.filter((front) => authUser.serviceFrontIds.includes(front.id)); }, [authUser, serviceFronts]);
    const filteredData = useMemo(() => { if (activeFrontId === "ALL")
        return data; return { ...data, equipment: data.equipment.filter((item) => item.serviceFrontId === activeFrontId), alerts: data.alerts.filter((item) => data.equipment.find((equipmentItem) => equipmentItem.id === item.equipmentId)?.serviceFrontId === activeFrontId), history: data.history.filter((item) => item.equipmentId === null || data.equipment.find((equipmentItem) => equipmentItem.id === item.equipmentId)?.serviceFrontId === activeFrontId), readings: data.readings.filter((item) => item.equipmentId === null || data.equipment.find((equipmentItem) => equipmentItem.id === item.equipmentId)?.serviceFrontId === activeFrontId) }; }, [data, activeFrontId]);
    if (authLoading)
    ...
    const standaloneAllowed = active === "Status da Frota" && can(authUser, "fleet.view") || active === "Equipamentos" && can(authUser, "equipment.view") || active === "Solicitação de Materiais" && can(authUser, "materials.view") || active === "Produtos" && can(authUser, "products.view") || active === "Tarefas" && can(authUser, "tasks.view") || active === "Usuários" && can(authUser, "users.view");
    const effective = standaloneAllowed ? active : !allowed.some(([, label]) => label === active) ? allowed[0]?.[1] ?? (can(authUser, "equipment.view") ? "Equipamentos" : can(authUser, "fleet.view") ? "Status da Frota" : can(authUser, "materials.view") ? "Solicitação de Materiais" : can(authUser, "products.view") ? "Produtos" : can(authUser, "tasks.view") ? "Tarefas" : can(authUser, "users.view") ? "Usuários" : "Dashboard") : active;
...
function SectionView(props: {
    ...
    return <FleetStatusView authUser={props.authUser} flash={props.flash}/>; if (props.section === "Solicitação de Materiais")
    return <MaterialRequestsView authUser={props.authUser} flash={props.flash}/>; if (props.section === "Produtos")
    return <ProductsView authUser={props.authUser} flash={props.flash}/>; if (props.section === "Tarefas")
...
function HistoryView({ data, authUser, refresh, flash, openEquipment, fronts: serviceFronts }: {
    data: SystemData;
    authUser: AuthUser;
    refresh: () => Promise<void>;
    flash: (message: string) => void;
    ...
    const canExportXlsx = authUser.profile === "ADMIN" || authUser.profile === "GESTOR" || authUser.canExport;
...
function ExportXlsxModal({ equipment, fronts, close }: { equipment: Equipment[]; fronts: ServiceFront[]; close: () => void }) {
    ...
    async function run() {
        ...
        try {
            ...
            const response = await fetch(`/api/maintenance-export-xlsx?${params.toString()}`, { cache: "no-store" });
            if (!response.ok) {
                const data = await response.json().catch(() => ({})) as { error?: string };
                setError(data.error ?? "Não foi possível gerar a exportação.");
                return;
            }
            ...
            link.href = url; link.download = filenameMatch?.[1] ?? "trocas-de-oleo.xlsx"; link.click();
            URL.revokeObjectURL(url);
            close();
        } catch (problem) {
            setError(problem instanceof Error ? problem.message : "Não foi possível gerar a exportação.");
        } finally {
            setBusy(false);
        }
    }

    return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className="modal">
        <header><div><p className="eyebrow">TROCAS DE ÓLEO</p><h2>Exportar Excel</h2><span>Gera uma planilha .xlsx com as trocas do período, respeitando as frentes que você enxerga.</span></div><button onClick={close}>×</button></header>
        <div className="modal-form">
            <label className="full">Período<div className="report-shortcut-row"><button type="button" onClick={() => applyShortcut("30D")}>Últimos 30 dias</button><button type="button" onClick={() => applyShortcut("MONTH")}>Mês atual</button><button type="button" onClick={() => applyShortcut("LAST_MONTH")}>Mês anterior</button><button type="button" onClick={() => applyShortcut("YEAR")}>Ano atual</button></div></label>
            <label>Data inicial<input type="date" required value={dateFrom} onChange={(event) => setDateFrom(event.target.value)}/></label>
            <label>Data final<input type="date" required value={dateTo} min={dateFrom} onChange={(event) => setDateTo(event.target.value)}/></label>
            <label>Frente de serviço<select value={frontId} onChange={(event) => setFrontId(event.target.value)}><option value="ALL">Todas as frentes que você enxerga</option>{fronts.map((front) => <option key={front.id} value={front.id}>{front.name}</option>)}</select></label>
            <label>Equipamento<select value={equipmentId} onChange={(event) => setEquipmentId(event.target.value)}><option value="">Todos</option>{equipment.map((item) => <option key={item.id} value={item.id}>{item.prefix} · {item.brand} {item.model}</option>)}</select></label>
            <label>Tipo de equipamento<select value={equipmentType} onChange={(event) => setEquipmentType(event.target.value)}><option value="">Todos</option>{types.map((type) => <option key={type}>{type}</option>)}</select></label>
            <label>Responsável<input value={responsible} onChange={(event) => setResponsible(event.target.value)} placeholder="Nome do responsável pela troca"/></label>
            {error && <div className="equipment-form-error full"><span>!</span><strong>{error}</strong></div>}
            <div className="modal-footer full"><button type="button" className="secondary" onClick={close}>Cancelar</button><button className="primary" disabled={busy} onClick={run}>{busy ? "Gerando..." : "Gerar planilha"}</button></div>
        </div>
    </section></div>;
}
...
function UsersView({ users, authUser, loadUsers, flash, openUserModal, taskRoles, loadTaskRoles, fronts }: {
    users: UserRecord[];
    authUser: AuthUser;
    loadUsers: () => Promise<void>;
    flash: (message: string) => void;
...
} } const [subView, setSubView] = useState<"list" | "hierarchy">("list"); const [managerOpen, setManagerOpen] = useState(false); const [resetPasswordUser, setResetPasswordUser] = useState<UserRecord | null>(null); const isAdmin = authUser.profile === "ADMIN"; return <><ModuleHeader eyebrow="ACESSO E SEGURANÇA" title="Usuários e permissões" subtitle="Este módulo permanece separado de Troca de Óleo." action={can(authUser, "users.create") ? "＋ Novo usuário" : undefined} onAction={() => openUserModal("new")}/><div className="user-summary"><article><strong>{users.length}</strong><span>Cadastrados</span></article><article><strong>{users.filter((user) => user.status === "ACTIVE").length}</strong><span>Ativos</span></article><article><strong>{users.filter((user) => user.profile === "ADMIN").length}</strong><span>Administradores</span></article><article><strong>{users.filter((user) => user.status === "INACTIVE").length}</strong><span>Inativos</span></article></div><div className="main-tabs secondary-module-nav" aria-label="Sub-navegação de Usuários"><button className={subView === "list" ? "active" : ""} onClick={() => setSubView("list")}>Lista</button><button className={subView === "hierarchy" ? "active" : ""} onClick={() => setSubView("hierarchy")}>Cargos de Tarefas</button>{isAdmin && <button className="task-role-manager-button" onClick={() => setManagerOpen(true)}>⚙ Gestor de Cargos de Tarefas</button>}</div>{subView === "list" ? <article className="panel module-panel"><div className="table-scroll"><table className="users-table"><thead><tr><th>Nome</th><th>Usuário</th><th>E-mail</th><th>Frente</th><th>Perfil</th><th>Cargo de Tarefas</th><th>Status</th><th>Último acesso</th><th>Ações</th></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td><div className="user-name-cell"><span className="avatar light">{user.name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</span><div><strong>{user.name}</strong>{user.isPrimaryAdmin && <small>Administrador principal</small>}</div></div></td><td>@{user.username}</td><td>{user.email}</td><td>{user.allServiceFronts ? <span className="user-front-badge">Todas</span> : user.serviceFrontNames.length > 0 ? user.serviceFrontNames.join(", ") : (user.serviceFrontName ?? "—")}</td><td><span className="role-pill">{user.profileLabel}</span></td><td>{user.taskRoleName ? <span className="role-pill">{user.taskRoleName}</span> : <span className="role-pill warning">Sem cargo</span>}</td><td><span className={`status-pill ${user.status === "ACTIVE" ? "green" : "gray"}`}>{user.status === "ACTIVE" ? "Ativo" : "Inativo"}</span></td><td>{user.lastAccessAt ? formatDate(user.lastAccessAt) : "Nunca"}</td><td><div className="user-actions">{can(authUser, "users.edit") && (!user.isPrimaryAdmin || user.id === authUser.id) && <button onClick={() => openUserModal(user)}>Editar</button>}{can(authUser, "users.edit") && (!user.isPrimaryAdmin || user.id === authUser.id) && <button onClick={() => setResetPasswordUser(user)}>Trocar senha</button>}{can(authUser, "users.status") && !user.isPrimaryAdmin && user.id !== authUser.id && <button className={user.status === "ACTIVE" ? "danger-action" : "activate-action"} onClick={() => toggle(user)}>{user.status === "ACTIVE" ? "Desativar" : "Ativar"}</button>}</div></td></tr>)}</tbody></table></div></article> : <TaskRoleGroupsView users={users} authUser={authUser} openUserModal={openUserModal} taskRoles={taskRoles}/>}{can(authUser, "service_fronts.manage") && <ServiceFrontManager fronts={fronts}/>}{managerOpen && <TaskRoleManagerModal close={() => setManagerOpen(false)} onSaved={loadTaskRoles}/>}{resetPasswordUser && <ResetPasswordModal user={resetPasswordUser} close={() => setResetPasswordUser(null)} saved={(message) => { setResetPasswordUser(null); flash(message); }}/>}</>; }

function ServiceFrontManager({ fronts }: { fronts: ServiceFront[] }) {
    const [rows, setRows] = useState<ServiceFront[]>(fronts);
    ...
    const load = useCallback(() => {
        fetchJson<{ fronts: ServiceFront[] }>("/api/service-fronts?includeInactive=1").then((result) => setRows(result.fronts)).catch(() => undefined);
    }, []);
    useEffect(() => { load(); }, [load]);
    ...
    async function create(event: FormEvent) {
        ...
        setBusy(true); setError("");
        try {
            await fetchJson("/api/service-fronts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, location }) });
            setName(""); setLocation(""); load();
        } catch (problem) {
            setError(problem instanceof Error ? problem.message : "Não foi possível cadastrar a frente.");
        } finally {
            setBusy(false);
        }
    }

    async function toggleActive(front: ServiceFront) {
        try {
            await fetchJson(`/api/service-fronts/${front.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active: !front.active }) });
            load();
        } catch (problem) {
            flashFallback(problem);
        }
    }
    function flashFallback(problem: unknown) { setError(problem instanceof Error ? problem.message : "Não foi possível atualizar a frente."); }

    return <details className="service-front-manager">
        <summary>Gerenciar frentes de serviço · {rows.length} cadastradas</summary>
        <form className="service-front-manager-row" onSubmit={create}>
            <input required placeholder="Nome da frente (ex.: MRN)" value={name} onChange={(event) => setName(event.target.value)}/>
            <input placeholder="Localização (opcional)" value={location} onChange={(event) => setLocation(event.target.value)}/>
            <button className="primary" disabled={busy}>{busy ? "Salvando..." : "＋ Adicionar"}</button>
        </form>
        {error && <div className="operation-error"><span>!</span><div><strong>Falha</strong><p>{error}</p></div></div>}
        <div className="service-front-manager-list">
            {rows.map((front) => <div className="service-front-manager-row" key={front.id}>
                <strong>{front.name}</strong>
                <span>{front.location ?? "—"}</span>
                <button onClick={() => toggleActive(front)}>{front.active ? "Desativar" : "Reativar"}</button>
            </div>)}
        </div>
    </details>;
}

function TaskRoleGroupsView({ users, authUser, openUserModal, taskRoles }: {
    users: UserRecord[];
    authUser: AuthUser;
...
function UserModal({ item, actor, fronts, taskRoles, close, saved }: {
    ...
    close: () => void;
    saved: (message: string) => void;
}) { const baseProfile = (item?.profile === "ALMOXARIFADO" ? "OPERADOR" : item?.profile ?? "OPERADOR") as "ADMIN" | "GESTOR" | "OFICINA" | "OPERADOR"; const [profile, setProfile] = useState(baseProfile); const [permissions, setPermissions] = useState<Permission[]>(item?.permissions ?? profileDefaults[baseProfile]); const [taskRoleId, setTaskRoleId] = useState(item?.taskRoleId ? String(item.taskRoleId) : ""); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const canEditTaskRole = actor.profile === "ADMIN" && item?.id !== actor.id; const selectedTaskRole = taskRoles.find((role) => role.id === Number(taskRoleId));
    const [allFronts, setAllFronts] = useState(item?.allServiceFronts ?? false);
    const [serviceFrontIds, setServiceFrontIds] = useState<number[]>(item?.serviceFrontIds ?? (item?.serviceFrontId ? [item.serviceFrontId] : []));
    const [primaryFrontId, setPrimaryFrontId] = useState(item?.serviceFrontId ? String(item.serviceFrontId) : "");
    const [canExport, setCanExport] = useState(item?.canExport ?? false);
    async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(""); const form = new FormData(event.currentTarget); const payload = { id: item?.id, action: "UPDATE", name: form.get("name"), username: form.get("username"), email: form.get("email"), password: item ? undefined : form.get("password"), profile, taskRoleId: taskRoleId ? Number(taskRoleId) : null, status: item?.status ?? "ACTIVE", serviceFrontId: primaryFrontId || null, allServiceFronts: allFronts, serviceFrontIds, canExport, permissions }; try {
    ...
    close(); }}><section className="modal user-modal"><header><div><p className="eyebrow">ACESSO E SEGURANÇA</p><h2>{item ? "Editar usuário" : "Novo usuário"}</h2><span>Defina perfil e permissões validadas no servidor.</span></div><button onClick={close}>×</button></header><form className="modal-form user-form" onSubmit={submit}><label>Nome<input name="name" required defaultValue={item?.name}/></label><label>Usuário<input name="username" required defaultValue={item?.username}/></label><label className="full">E-mail<input name="email" type="email" required defaultValue={item?.email}/></label>{!item && <label className="full">Senha inicial<input name="password" type="password" minLength={10} required/></label>}<label>Perfil<select value={profile} disabled={!assign && Boolean(item)} onChange={(event) => { const next = event.target.value as typeof profile; setProfile(next); setPermissions(profileDefaults[next]); }}><option value="ADMIN">Administrador</option><option value="GESTOR">Gestor</option><option value="OFICINA">Manutenção / Oficina</option><option value="OPERADOR">Operador</option></select></label><label>Cargo de Tarefas<select value={taskRoleId} disabled={!canEditTaskRole} onChange={(event) => setTaskRoleId(event.target.value)}><option value="">— Não definido —</option>{taskRoles.map((role) => <option key={role.id} value={role.id}>{role.name}{role.isRoot ? " (raiz)" : ""}</option>)}</select>{!canEditTaskRole && <small>Somente o administrador altera o Cargo de Tarefas de outra pessoa — e ninguém altera o próprio.</small>}{selectedTaskRole && <small>Define quem este usuário pode enviar/receber/visualizar/gerenciar tarefas, conforme o Gestor de Cargos de Tarefas.</small>}{!taskRoleId && <small className="warning-text">Sem Cargo de Tarefas, este usuário não poderá criar nem receber tarefas.</small>}</label><fieldset className="service-fronts-selector full"><legend>Frentes de serviço</legend>{profile==="ADMIN" ? <small>Administrador enxerga todas as frentes automaticamente.</small> : <>
    <label className="service-fronts-all"><input type="checkbox" disabled={!assign} checked={allFronts} onChange={(event) => setAllFronts(event.target.checked)}/><span><b>Acesso a todas as frentes</b><small>O usuário verá todas as frentes, inclusive as cadastradas futuramente.</small></span></label>
    {!allFronts && <div className="service-fronts-list">{fronts.map((front) => <label key={front.id} className={serviceFrontIds.includes(front.id) ? "checked" : ""}><input type="checkbox" disabled={!assign} checked={serviceFrontIds.includes(front.id)} onChange={(event) => setServiceFrontIds((current) => event.target.checked ? [...current, front.id] : current.filter((id) => id !== front.id))}/><span>{front.name}</span></label>)}</div>}
    <label className="primary-front-field">Frente principal<select name="serviceFrontId" required disabled={!assign} value={primaryFrontId} onChange={(event) => setPrimaryFrontId(event.target.value)}><option value="" disabled>Selecione</option>{(allFronts ? fronts : fronts.filter((front) => serviceFrontIds.includes(front.id))).map((front) => <option key={front.id} value={front.id}>{front.name}</option>)}</select><small>Padrão em formulários e relatórios.</small></label>
    <label className="can-export-toggle"><input type="checkbox" disabled={!assign} checked={canExport} onChange={(event) => setCanExport(event.target.checked)}/><span><b>Pode exportar trocas de óleo em Excel</b><small>ADMIN e GESTOR sempre podem exportar; esta opção libera para os demais perfis.</small></span></label>
  </>}</fieldset><div className="permission-groups full">{permissionGroups.map((group) => <fieldset key={group.label}><legend>{group.label}</legend>{group.items.map(([permission, label]) => <label className={permissions.includes(permission) ? "checked" : ""} key={permission}><input type="checkbox" disabled={!assign && Boolean(item)} checked={permissions.includes(permission)} onChange={(event) => setPermissions((current) => event.target.checked ? [...current, permission] : current.filter((key) => key !== permission))}/><span><b>{label}</b><small>{permissions.includes(permission) ? "Permitido" : "Bloqueado"}</small></span><i>{permissions.includes(permission) ? "✓" : ""}</i></label>)}</fieldset>)}</div>{error && <div className="equipment-form-error full"><span>!</span><strong>{error}</strong></div>}<div className="modal-footer full"><button type="button" className="secondary" onClick={close}>Cancelar</button><button className="primary" disabled={busy}>{busy ? "Salvando..." : "Salvar usuário"}</button></div></form></section></div>; }

function ResetPasswordModal({ user, close, saved }: { user: UserRecord; close: () => void; saved: (message: string) => void }) {
    ...
    async function submit(event: FormEvent) {
        ...
        setBusy(true); setError("");
        try {
            await fetchJson("/api/users", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: user.id, action: "RESET_PASSWORD", password }) });
            saved(`Senha de "${user.name}" alterada.`);
        }
        catch (problem) {
            setError(problem instanceof Error ? problem.message : "Não foi possível trocar a senha.");
        }
        finally {
            setBusy(false);
        }
    }
    return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className="modal"><header><div><p className="eyebrow">ACESSO E SEGURANÇA</p><h2>Trocar senha</h2><span>{user.name} (@{user.username})</span></div><button onClick={close}>×</button></header><form className="modal-form" onSubmit={submit}><label className="full">Nova senha<input type="password" required minLength={10} value={password} onChange={(event) => setPassword(event.target.value)}/><small>Use letra maiúscula, minúscula, número e símbolo, com pelo menos 10 caracteres.</small></label><label className="full">Confirmar senha<input type="password" required minLength={10} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)}/></label>{error && <div className="equipment-form-error full"><span>!</span><strong>{error}</strong></div>}<div className="modal-footer full"><button type="button" className="secondary" onClick={close}>Cancelar</button><button className="primary" disabled={busy}>{busy ? "Salvando..." : "Salvar nova senha"}</button></div></form></section></div>;
}
