...
type Section = "Dashboard" | "Equipamentos da troca" | "Equipamentos" | "QR Codes" | "Horímetros / KM" | "Registrar troca de óleo" | "Central de alertas" | "WhatsApp" | "Histórico" | "Status da Frota" | "Solicitação de Materiais" | "Tarefas" | "Usuários";
...
const permissionGroups: Array<{
    ...
    { label: "Usuários", items: [["users.view", "Visualizar usuários"], ["users.create", "Criar usuários"], ["users.edit", "Editar usuários"], ["users.permissions", "Alterar permissões"], ["users.status", "Ativar/desativar"], ["users.delete", "Excluir usuários"]] },
];
const allPermissions = permissionGroups.flatMap((group) => group.items.map(([key]) => key));
const profileDefaults: Record<"ADMIN" | "GESTOR" | "OFICINA" | "OPERADOR", Permission[]> = { ADMIN: allPermissions, GESTOR: ["dashboard.view", "equipment.view", "meter.view", "maintenance.view", "maintenance.history", "alerts.view", "alerts.share", "whatsapp.view", "whatsapp.send", "fleet.view", "fleet.update", "fleet.report", "materials.view", "materials.manage", "tasks.view", "tasks.create", "tasks.edit"], OFICINA: ["equipment.view", "equipment.edit_plan", "meter.view", "meter.create", "maintenance.view", "maintenance.create", "maintenance.edit", "maintenance.history", "alerts.view", "fleet.view", "fleet.update", "fleet.report", "materials.view", "materials.request", "tasks.view", "tasks.create", "tasks.edit"], OPERADOR: [] };
...
export default function Home() {
    ...
    const standaloneAllowed = active === "Status da Frota" && can(authUser, "fleet.view") || active === "Equipamentos" && can(authUser, "equipment.view") || active === "Solicitação de Materiais" && can(authUser, "materials.view") || active === "Tarefas" && can(authUser, "tasks.view") || active === "Usuários" && can(authUser, "users.view");
    const effective = standaloneAllowed ? active : !allowed.some(([, label]) => label === active) ? allowed[0]?.[1] ?? (can(authUser, "equipment.view") ? "Equipamentos" : can(authUser, "fleet.view") ? "Status da Frota" : can(authUser, "materials.view") ? "Solicitação de Materiais" : can(authUser, "tasks.view") ? "Tarefas" : can(authUser, "users.view") ? "Usuários" : "Dashboard") : active;
...
function SectionView(props: {
    ...
    return <MaterialRequestsView authUser={props.authUser} flash={props.flash}/>; if (props.section === "Tarefas")
...
} } async function remove(user: UserRecord) { if (!window.confirm(`Excluir o usuário "${user.name}"? Ele perde o acesso imediatamente, mas o histórico é preservado e um administrador pode restaurar depois.`)) return; try { const result = await fetchJson<{ message: string }>("/api/users", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: user.id, action: "DELETE" }) }); await loadUsers(); if (showDeleted) await loadDeleted(); flash(result.message); } catch (problem) { flash(problem instanceof Error ? problem.message : "Não foi possível excluir o usuário."); } } async function restore(user: UserRecord) { try { const result = await fetchJson<{ message: string }>("/api/users", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: user.id, action: "RESTORE" }) }); await loadUsers(); await loadDeleted(); flash(result.message); } catch (problem) { flash(problem instanceof Error ? problem.message : "Não foi possível restaurar o usuário."); } } const [subView, setSubView] = useState<"list" | "hierarchy">("list"); const [managerOpen, setManagerOpen] = useState(false); const [showDeleted, setShowDeleted] = useState(false); const [deletedUsers, setDeletedUsers] = useState<UserRecord[]>([]); const loadDeleted = useCallback(async () => { try { const result = await fetchJson<{ users: UserRecord[] }>("/api/users?includeDeleted=1"); setDeletedUsers(result.users.filter((item) => item.deletedAt)); } catch (problem) { flash(problem instanceof Error ? problem.message : "Não foi possível carregar os usuários excluídos."); } }, [flash]); useEffect(() => { if (showDeleted) loadDeleted(); }, [showDeleted, loadDeleted]); const rows = showDeleted ? [...users, ...deletedUsers] : users; const isAdmin = authUser.profile === "ADMIN"; return <><ModuleHeader eyebrow="ACESSO E SEGURANÇA" title="Usuários e permissões" subtitle="Este módulo permanece separado de Troca de Óleo." action={can(authUser, "users.create") ? "＋ Novo usuário" : undefined} onAction={() => openUserModal("new")}/><div className="user-summary"><article><strong>{users.length}</strong><span>Cadastrados</span></article><article><strong>{users.filter((user) => user.status === "ACTIVE").length}</strong><span>Ativos</span></article><article><strong>{users.filter((user) => user.profile === "ADMIN").length}</strong><span>Administradores</span></article><article><strong>{users.filter((user) => user.status === "INACTIVE").length}</strong><span>Inativos</span></article></div><div className="main-tabs secondary-module-nav" aria-label="Sub-navegação de Usuários"><button className={subView === "list" ? "active" : ""} onClick={() => setSubView("list")}>Lista</button><button className={subView === "hierarchy" ? "active" : ""} onClick={() => setSubView("hierarchy")}>Cargos de Tarefas</button>{isAdmin && <button className="task-role-manager-button" onClick={() => setManagerOpen(true)}>⚙ Gestor de Cargos de Tarefas</button>}</div>{subView === "list" ? <article className="panel module-panel">{can(authUser, "users.delete") && <label className="compact-switch deleted-users-switch"><input type="checkbox" checked={showDeleted} onChange={(event) => setShowDeleted(event.target.checked)}/><span>Mostrar excluídos{deletedUsers.length > 0 ? ` (${deletedUsers.length})` : ""}</span></label>}<div className="table-scroll"><table className="users-table"><thead><tr><th>Nome</th><th>Usuário</th><th>E-mail</th><th>Frente</th><th>Perfil</th><th>Cargo de Tarefas</th><th>Status</th><th>Último acesso</th><th>Ações</th></tr></thead><tbody>{rows.map((user) => <tr key={user.id} className={user.deletedAt ? "deleted-row" : ""}><td><div className="user-name-cell"><span className="avatar light">{user.name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</span><div><strong>{user.name}</strong>{user.isPrimaryAdmin && <small>Administrador principal</small>}</div></div></td><td>@{user.username}</td><td>{user.email}</td><td>{user.serviceFrontName ?? "Todas as frentes"}</td><td><span className="role-pill">{user.profileLabel}</span></td><td>{user.taskRoleName ? <span className="role-pill">{user.taskRoleName}</span> : <span className="role-pill warning">Sem cargo</span>}</td><td>{user.deletedAt ? <span className="status-pill red">Excluído</span> : <span className={`status-pill ${user.status === "ACTIVE" ? "green" : "gray"}`}>{user.status === "ACTIVE" ? "Ativo" : "Inativo"}</span>}</td><td>{user.lastAccessAt ? formatDate(user.lastAccessAt) : "Nunca"}</td><td><div className="user-actions">{user.deletedAt ? (can(authUser, "users.delete") && <button className="activate-action" onClick={() => restore(user)}>Restaurar</button>) : <>{can(authUser, "users.edit") && (!user.isPrimaryAdmin || user.id === authUser.id) && <button onClick={() => openUserModal(user)}>Editar</button>}{can(authUser, "users.status") && !user.isPrimaryAdmin && user.id !== authUser.id && <button className={user.status === "ACTIVE" ? "danger-action" : "activate-action"} onClick={() => toggle(user)}>{user.status === "ACTIVE" ? "Desativar" : "Ativar"}</button>}{can(authUser, "users.delete") && !user.isPrimaryAdmin && user.id !== authUser.id && <button className="danger-action" onClick={() => remove(user)}>Excluir</button>}</>}</div></td></tr>)}</tbody></table></div></article> : <TaskRoleGroupsView users={users} authUser={authUser} openUserModal={openUserModal} taskRoles={taskRoles}/>}{managerOpen && <TaskRoleManagerModal close={() => setManagerOpen(false)} onSaved={loadTaskRoles}/>}</>; }
